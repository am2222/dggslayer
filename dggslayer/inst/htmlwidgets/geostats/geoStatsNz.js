class geostats {
	constructor(a) {
		...
		// this.stat_sorted = a.stat_sorted;
		this.stat_mean = a.avg;
		this.stat_median = a.median;
		this.stat_sum = a.sum;
		this.stat_max = a.max;
		this.stat_min = a.min;
		this.stat_pop = a.count;
		this.stat_variance = a.var_pop;
		// this.stat_stddev	= a.stat_stddev;
		// this.stat_cov		= a.stat_cov;
		this.hasNegativeValue = (a.min<0)? true:false;
		// this.hasZeroValue = a.hasZeroValue;
	}
    /**
         * logging method
         */
	...
	_nbClassInt(nbClass) {
		...
		if (isNaN(nbclassTmp)) {
			if (this.silent)
				this.log("[silent mode] '" + nbclassTmp + "' is not a valid integer. Enable to set class number.", true);
			else
				throw new TypeError("'" + nbclassTmp + "' is not a valid integer. Enable to set class number.");
		}
		else {
			return nbclassTmp;
		}
	...
	info() {
		if (this._nodata())
			return;
		var content = '';
		content += ('Population') + ' : ' + this.pop() + ' - [' + ('Min')
			+ ' : ' + this.min() + ' | ' + ('Max') + ' : ' + this.max()
			+ ']' + "\n";
		content += ('Mean') + ' : ' + this.mean() + ' - ' + ('Median') + ' : ' + this.median() + "\n";
		content += ('Variance') + ' : ' + this.variance() + ' - ' + ('Standard deviation') + ' : ' + this.stddev()
			+ ' - ' + ('Coefficient of variation') + ' : ' + this.cov() + "\n";
		return content;
	}
	getPrecisionFlag(){
    ...
    /**
         ...
         *
         */
	setClassManually(array) {
		if (this._nodata())
			return;
		if (array[0] !== this.min() || array[array.length - 1] !== this.max()) {
			if (this.silent)
				this.log("[silent mode] " + t('Given bounds may not be correct! please check your input.\nMin value : ' + this.min() + ' / Max value : ' + this.max()), true);
			else
				throw new TypeError(('Given bounds may not be correct! please check your input.\nMin value : ' + this.min() + ' / Max value : ' + this.max()));
			return;
		}
		this.setBounds(array);
		this.setRanges();
		// we specify the classification method
		this.method = ('manual classification') + ' (' + (array.length - 1) + ' ' + ('classes') + ')';
		return this.bounds;
	}
	getPrecision(){
    ...
    /**
         ...
         * 0.75, 1.5, 2.25, 3);
         */
	getClassEqInterval(nbClass, forceMin, forceMax) {
		...
		this.setRanges();
		// we specify the classification method
		this.method = ('eq. intervals') + ' (' + nbClass + ' ' + ('classes') + ')';
		return this.bounds;
	}
    ...
    /**
         ...
         * 1.5, 2.25, 3);
         */
	getClassQuantile(nbClass) {
		nbClass = this._nbClassInt(nbClass); // ensure nbClass is an integer
		// if (this._nodata())
		// 	return;
		// var tmp = this.sorted();
		var bounds = this.getQuantiles(nbClass);
		bounds.unshift(this.min);
		if (bounds[this.pop - 1] !== this.max)
			bounds.push(this.max);
		this.setBounds(bounds);
		this.setRanges();
		// we specify the classification method
		this.method = ('quantile') + ' (' + nbClass + ' ' + ('classes') + ')';
		return this.bounds;
	}
    /**
         ...
         * 0.75, 1.5, 2.25, 3);
         */
	getClassStdDeviation(nbClass, matchBounds) {
		nbClass = this._nbClassInt(nbClass); // ensure nbClass is an integer
		if (this._nodata())
			return;
		var tmpMax = this.max();
		...
		this.setRanges();
		// we specify the classification method
		this.method = ('std deviation') + ' (' + nbClass + ' ' + ('classes') + ')';
		return this.bounds;
	}
    /**
         ...
         * 0.75, 1.5, 2.25, 3);
         */
	getClassGeometricProgression(nbClass) {
		nbClass = this._nbClassInt(nbClass); // ensure nbClass is an integer
		if (this._nodata())
			return;
		if (this._hasNegativeValue() || this._hasZeroValue()) {
			if (this.silent)
				this.log("[silent mode] " + ('geometric progression can\'t be applied with a serie containing negative or zero values.'), true);
			else
				throw new TypeError(('geometric progression can\'t be applied with a serie containing negative or zero values.'));
			return;
		}
		var a = Array();
		var tmpMin = this.min();
		...
		this.setRanges();
		// we specify the classification method
		this.method = ('geometric progression') + ' (' + nbClass + ' ' + ('classes') + ')';
		return this.bounds;
	}
    /**
         ...
         * 0.75, 1.5, 2.25, 3);
         */
	getClassArithmeticProgression(nbClass) {
		nbClass = this._nbClassInt(nbClass); // ensure nbClass is an integer
		if (this._nodata())
			return;
		var denominator = 0;
		...
		this.setRanges();
		// we specify the classification method
		this.method = ('arithmetic progression') + ' (' + nbClass + ' ' + ('classes') + ')';
		return this.bounds;
	}
    /**
	...
	getClassJenks2(nbClass) {
		...
		this.setRanges();
		this.method = ('Jenks2') + ' (' + nbClass + ' ' + ('classes') + ')';
	...
	getClassJenks(nbClass) {
		...
		jenksMatrices = function (data, n_classes) {
			...
			var lower_class_limits = [], variance_combinations = [],
				// loop counters
				i, j,
		...
		var matrices = jenksMatrices(dataList, nbClass),
			...
			lower_class_limits = matrices.lower_class_limits, k = dataList.length - 1, kclass = [], countNum = nbClass;
		...
		this.setBounds(kclass);
		this.setRanges();
		this.method = ('Jenks') + ' (' + nbClass + ' ' + ('classes') + ')';
		return this.bounds; //array of breaks
	}
    // /**
    //      * Quantile classification Return an array with bounds : ie array(0, 0.75,
	...
	getClass(value) {
		...
		//FIXME:return default value
		return ("Unable to get value's class.");
	}
	getColor(value){
		return this.colors[this.getClass(value)];
	}
    /**
	...
	getInnerRanges() {
		...
		for (var i = 0; i < tmp.length; i++) {
			if (i == 0)
				var range_firstvalue = tmp[i]; // we init first range value
	...
	getHtmlLegend(colors, legend, counter, callback, mode, order) {
		...
		if (mode == 'discontinuous') {
			...
			if (this.counter.indexOf(0) !== -1) {
				if (this.silent)
					...
					throw new TypeError(("Geostats cannot apply 'discontinuous' mode to the getHtmlLegend() method because some classes are not populated.\nPlease switch to 'default' or 'distinct' modes. Exit!"));
				return;
			}
		}
		if (order !== 'DESC')
		...
		if (ccolors.length < this.ranges.length) {
			if (this.silent)
				this.log("[silent mode] " + ('The number of colors should fit the number of ranges. Exit!'), true);
			else
				throw new TypeError(('The number of colors should fit the number of ranges. Exit!'));
			return;
		}
		if (this.is_uniqueValues == false) {
			for (var i = 0; i < (this.ranges.length); i++) {
		...
		var content = '<div class="geostats-legend"><div class="geostats-legend-title">' + (lg) + '</div>';
